...
contract L1BossBridgeTest is Test {
    ...
    function testCanMoveApprovedTokensOfOtherUsers() public {
        ...
        //bob swoops in and steal
        uint256 depositAmount = token.balanceOf(user);
        address attacker = makeAddr("attacker"); //creating an attacker address
    ...
    function testCanTransferFromVaultToVault() public {
        ...
        deal(address(token), address(vault), vaultBalance); // this will give the Vault the 500 ether from above
    ...
    function testSignatureReplay() public {
        ...
        // Send tokens back to L1

        // The signer/Operator is going to sign the withdrawal
        bytes memory message = abi.encode(address(token), 0, abi.encodeCall(IERC20.transferFrom, (address(vault), attacker, attackerInitialBalance))); // we hash the message
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(operator.key, MessageHashUtils.toEthSignedMessageHash(keccak256(message))); // we use this to hash it into the eth format using keccak256. 
